...
def build_tmp_kb(tmpdir):
    ...
    conn.executemany(
        ...
        [('001', '人教七上', None, '版本', 1, '现行'),
         ...
         # 🔴 2026-08-21 窗L 就近可挂：考点长了题型子层 → 自己失去挂载资格，题型末端才收
         ('001001004', '角的概念', '001001', '考点', 4, '现行'),          # 已细分 → 拒
         ('001001004001', '角的个数探究', '001001004', '题型', 1, '现行'),  # 题型末端 → 过
         ('001001004002', '钟表的夹角问题', '001001004', '题型', 2, '退役'),  # 退役题型 → 拒
         ('001001005', '余角和补角', '001001', '考点', 5, '现行'),         # 子节点全退役 → 重新是末端
         ('001001005001', '已并走的题型', '001001005', '题型', 1, '退役')])
    conn.commit()
    return conn
...
def run_leaf(conn, rows_out):
    ...
    for name, kid in [('L1 叶子 001001001', '001001001'), ('L2 叶子 001001002', '001001002'),
                      ('L3 题型末端 001001004001', '001001004001'),
                      ('L4 子全退役复为末端 001001005', '001001005')]:
        try:
    ...
    for name, kid in [('X1 章级 001001', '001001'), ('X2 版本级 001', '001'),
                      ...
                      ('X7 已细分考点 001001004', '001001004'),
                      ('X8 退役题型 001001004002', '001001004002')]:
        try:
            assert_leaf_kp(conn, kid)
            ok, detail = False, '❌过了'
        except LeafKpError as e:
    ...
    # X7 的话术必须点名可挂的子节点（就近可挂：告诉调用方改挂哪，不是只说不行）
    try:
        assert_leaf_kp(conn, '001001004')
        ok, detail = False, '❌过了'
    except LeafKpError as e:
        ok = '001001004001' in str(e) and '角的个数探究' in str(e)
        detail = ('列出子节点' if ok else '❌话术没列子节点:' + str(e)[:60])
    ok_all &= ok
    rows_out.append(('闸②叶子', 'X7话术 列出可挂题型', '拒且指路', detail, ok))
    return ok_all
