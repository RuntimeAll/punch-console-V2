...
## 0c. PDF → 图片 通用件（2026-08-21 抽出，发布配图/目检共用）

```powershell
python 工具箱\渲染\pdf2img.py <PDF|目录|通配符...> --out <目录> [--pages all|first|last|1-3|1,3,5|-1]
       [--dpi 150] [--命名 原名|序号|"{卷}-p{页}"] [--起号 N] [--清空] [--清单] [--无水印] [--宽 N]
```
- 输入按给定顺序处理，`--命名 序号` 时编号顺序＝输入顺序（发布配图直接编成 `01.png…`）；
- 🔴 三条闸：**越界页不静默**（少出图退出码 2，除非 `--允许越界`）／**序号模式撞旧图拒跑**
  （两批混一个目录 = 发出去顺序全乱）／**水印默认开**（干净件要显式 `--无水印`）；
- 水印默认 `玉米训练营` #7a7a7a alpha 24，可 `--水印/--水印色/--水印透明/--水印疏密` 调。
- 目检用 `--无水印 --清单`；发布配图用默认水印 + `--命名 序号`。
