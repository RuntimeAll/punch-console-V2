...
契约（认知/数据结构.md §2.2 KG 口径定稿 2026-08-18）：
  ...
  · 别名表=翻译层：产线词/教辅原词/讲义词挂 kp_alias，alias_kind 记词源；
    🔴 对齐-002（2026-08-19 用户裁决）**只收正向产线词**——录入/出题时原词 resolve 到现行叶。
    不为历史数据铸兼容别名：历史数据对不上现行叶=弃或人工重标（"错误就是错误，不特别兼容"）。
    存量老区名别名不动（已在用），新增一律正向词；
